...
from app import db
UPLOAD_FOLDER = '/app/uploads'  # Pasta dentro do container
...
def get_veiculos():
    if request.method == "GET":
        ...
        placas = []
        pl = veiculos.veiculos.query.all()
        for veiculo in pl:
    ...
    if request.method == "POST":
        ...
        novo_veiculo = veiculos.veiculos(placa=placa, modelo=modelo, ano=ano, capacidade=capacidade)
...
def checklist():
    ...
    for campo in ["fotoFrontal", "fotoTraseira", "fotoLateral1", "fotoLateral2"]:
        ...
        if arquivo:
            ...
            filename = f"{placa}_{campo}{ext}"  # novo nome
